...
class Attention(nn.Module):
    def __init__(self, config, vis):
        ...
        self.attention_head_size = int(config["dim"] / self.num_attention_heads)
        self.all_head_size = self.num_attention_heads * self.attention_head_size

        self.query = Linear(config["dim"], config["dim"])
        self.key = Linear(config["dim"], config["dim"])
        self.value = Linear(config["dim"], config["dim"])

        self.out = Linear(config["dim"], config["dim"])
        self.attn_dropout = Dropout(config["p_drop_attn"])
        self.proj_dropout = Dropout(config["p_drop_attn"])
...
class Mlp(nn.Module):
    def __init__(self, config):
        ...
        self.fc1 = Linear(config['dim'], config["dim_ff"])
        self.fc2 = Linear(config["dim_ff"], config['dim'])
...
class Encoder(nn.Module):
    def __init__(self, config, vis):
        ...
        for _ in range(config["num_v_layers"]):
...
class VisionTransformerHash(nn.Module):
    def __init__(self, config, zero_head=True, vis=False):
